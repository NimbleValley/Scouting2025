...
function setUpCompare() {
    ...
    compareDescriptionContainer.innerHTML = `<p style="font-weight: bold">Compare</p>   <div><label for='compare-data-type'>Type:</label> <select id='compare-data-type' style='width: fit-content'>    <option value='mean'>Mean</option>   <option value='median'>Median</option>  <option value='max'>Max</option>  <option value='min'>Min</option>     </select></div>`;
    ...
    let statContainers = [];
    for (let i = 0; i < TEAM_FIELDS_ORDER.length - 1; i++) {
        ...
        tempStatName.innerText = camelCaseToWords(TEAM_FIELDS_ORDER[i + 1]);
    ...
    document.getElementById('compare-data-type').addEventListener('change', function () { 
        localStorage.setItem('compare-data-type', document.getElementById('compare-data-type').value);
...
function doCompare(teamSelects, statContainers) {
    ...
    let mode = localStorage.getItem('compare-data-type');
    if(mode == null) {
        mode = 'mean'
    }
    ...
    for (let i = 0; i < statContainers.length; i++) {
        ...
        for (let t = 0; t < tempNumbers.length; t++) {
            tempNumbers[t].innerText = TEAM_COLUMNS[mode][i + 1][teamIndices[t]];

            if (TEAM_COLUMNS[mode][i + 1][teamIndices[t]] == 0) {
                teamStats.push(0.1);
            } else {
                teamStats.push(TEAM_COLUMNS[mode][i + 1][teamIndices[t]]);
            }
        }
        ...
        for (let l = 0; l < 2; l++) {
            ...
            if (badCompareValues.includes(TEAM_FIELDS_ORDER[i+1])) {
                if (width > 50) {
